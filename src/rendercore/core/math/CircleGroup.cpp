...
void CircleGroup::add_circle(const Circle &circle, const Color &color) {
...
void CircleGroup::build_mesh() {
    ...
    const ArrayBufferAttribute attribute = ArrayBufferAttribute(3, 1).set_divisor(1);
    buf->add_attribute(attribute);

    std::vector<float> color_data(circles.size() * 3);
    for (int i = 0; i < circles.size(); ++i) {
        data[i*3] = colors[i].r;
        data[i*3 + 1] = colors[i].g;
        data[i*3 + 2] = colors[i].b;
        data[i*3 + 3] = colors[i].a;
    }
    const std::shared_ptr<ArrayBuffer> color_buf = ArrayBuffer::create(color_data);
    const ArrayBufferAttribute color_attribute = ArrayBufferAttribute(4, 2).set_divisor(1);
